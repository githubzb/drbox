...
void DR_RGB2HSL(CGFloat r, CGFloat g, CGFloat b,
...
void DR_HSL2RGB(CGFloat h, CGFloat s, CGFloat l,
...
void DR_RGB2HSB(CGFloat r, CGFloat g, CGFloat b,
    ...
    *ss = delta / max;       // Saturation
    
    if (r == max) *hh = (g - b) / delta / 6;             // color between y & m
    else if (g == max) *hh = (2 + (b - r) / delta) / 6;  // color between c & y
    else *hh = (4 + (r - g) / delta) / 6;                // color between m & c
    if (*hh < 0) *hh += 1;
}

void DR_HSB2RGB(CGFloat hh, CGFloat ss, CGFloat bb,
                CGFloat *r, CGFloat *g, CGFloat *b) {
    CLAMP_COLOR_VALUE(hh);
    CLAMP_COLOR_VALUE(ss);
    CLAMP_COLOR_VALUE(bb);
    
    if (ss == 0) {
        *r = *g = *b = bb; // No Saturation, so Hue is undefined (Achromatic)
    } else {
        int sextant;
        CGFloat f, p, q, t;
        if (hh == 1) hh = 0;
        ...
        t = bb * (1 - ss * (1 - f));
        switch (sextant) {
            case 0: *r = bb; *g = t; *b = p; break;
            case 1: *r = q; *g = bb; *b = p; break;
            case 2: *r = p; *g = bb; *b = t; break;
            case 3: *r = p; *g = q; *b = bb; break;
            case 4: *r = t; *g = p; *b = bb; break;
            case 5: *r = bb; *g = p; *b = q; break;
        }
    }
}

void DR_RGB2CMYK(CGFloat r, CGFloat g, CGFloat b,
...
void DR_CMYK2RGB(CGFloat c, CGFloat m, CGFloat y, CGFloat k,
                 CGFloat *r, CGFloat *g, CGFloat *b) {
    CLAMP_COLOR_VALUE(c);
    CLAMP_COLOR_VALUE(m);
    CLAMP_COLOR_VALUE(y);
...
void DR_HSB2HSL(CGFloat h, CGFloat s, CGFloat b,
...
void DR_HSL2HSB(CGFloat h, CGFloat s, CGFloat l,
                CGFloat *hh, CGFloat *ss, CGFloat *bb) {
    CLAMP_COLOR_VALUE(h);
    CLAMP_COLOR_VALUE(s);
    CLAMP_COLOR_VALUE(l);
...
+ (UIColor *)dr_colorWithHue:(CGFloat)hue
                  saturation:(CGFloat)saturation
                   lightness:(CGFloat)lightness
                       alpha:(CGFloat)alpha {
    CGFloat r, g, b;
    DR_HSL2RGB(hue, saturation, lightness, &r, &g, &b);
...
+ (UIColor *)dr_colorWithCyan:(CGFloat)cyan
                      magenta:(CGFloat)magenta
                       yellow:(CGFloat)yellow
                        ...
                        alpha:(CGFloat)alpha {
    CGFloat r, g, b;
    DR_CMYK2RGB(cyan, magenta, yellow, black, &r, &g, &b);
    return [UIColor colorWithRed:r green:g blue:b alpha:alpha];
}
...
static BOOL hexStrToRGBA(NSString *str,
    ...
    str = [[str dr_trim] uppercaseString];
    if ([str hasPrefix:@"#"]) {
...
+ (instancetype)dr_colorWithHexString:(NSString *)hexStr {
    CGFloat r, g, b, a;
...
- (NSString *)dr_hexString {
    return [self dr_hexStringWithAlpha:NO];
}

- (NSString *)dr_hexStringWithAlpha {
    return [self dr_hexStringWithAlpha:YES];
}

- (NSString *)dr_hexStringWithAlpha:(BOOL)withAlpha {
    ...
    if (hex && withAlpha) {
        hex = [hex stringByAppendingFormat:@"%02lx",
               (unsigned long)(self.dr_alpha * 255.0 + 0.5)];
    }
    return hex;
}
...
- (UIColor *)dr_colorByChangeHue:(CGFloat)h saturation:(CGFloat)s brightness:(CGFloat)b alpha:(CGFloat)a {
    CGFloat hh, ss, bb, aa;
...
- (BOOL)dr_getHue:(CGFloat *)hue
    ...
    DR_RGB2HSL(r, g, b, hue, saturation, lightness);
    *alpha = a;
    return YES;
}
...
- (BOOL)dr_getCyan:(CGFloat *)cyan
    ...
    DR_RGB2CMYK(r, g, b, cyan, magenta, yellow, black);
    *alpha = a;
    return YES;
}
...
- (CGFloat)dr_saturation {
    CGFloat h, s = 0, b, a;
    [self getHue:&h saturation:&s brightness:&b alpha:&a];
    return s;
}

- (CGFloat)dr_brightness {
    CGFloat h, s, b = 0, a;
    [self getHue:&h saturation:&s brightness:&b alpha:&a];
    return b;
}

- (CGColorSpaceModel)dr_colorSpaceModel {
    return CGColorSpaceGetModel(CGColorGetColorSpace(self.CGColor));
}

- (NSString *)dr_colorSpaceString {
